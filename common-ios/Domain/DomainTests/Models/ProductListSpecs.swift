...
class ProductListSpecs: QuickSpec {
    override func spec() {
        var sut: ProductList!
        ...
        beforeEach {
            let product = Product(uid: "4878bf592579410fba52941d00b62f94",
                                  ...
                                  imageUrlsThumbnails: [])
            sut = ProductList(products: [product])
        }
        
        it("returns name") {
            expect(sut.products.first?.name) == "Notebook"
        }
        
        it("returns price") {
            expect(sut.products.first?.price) == "AED 5"
        }
    }
}
